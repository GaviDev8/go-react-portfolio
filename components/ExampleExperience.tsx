          ...
          {timelineData.data.map((item, index) => (
            <div key={`timeline-item-${index}`} className="flex items-center">
              <div className="flex justify-center items-center text-white rounded-full w-20 h-20 bg-slate-600">
                <div className="flex h-16 w-16 bg-slate-800 font-bold rounded-full justify-center items-center">
                  {formatDateRange(item.startMonth, item.startYear, item.endMonth, item.endYear)}
                </div>
              </div>
              <div className="ml-6">
                <div className="font-bold">
                  {item.title} @ <span className="text-purple font-bold">{item.company}</span>
                </div>
